...
def inject_dark_theme_css_legacy() -> None:
    st.markdown(
        """
        <style>
        :root {
...
def inject_8bit_ipn_theme_css() -> None:
    st.markdown(
        """
        <style>
        :root {
            --ipn: #5e0f20; --ipn-deep: #2e0717; --ipn-hot: #8d1c39;
            --gold: #c8a05a; --copper: #d79a4e; --cream: #fcf0dc;
            --void: #0d0f0f; --surface: #191b1b; --raised: #242626; --line: #76502c;
            --green: #4caf50; --amber: #e0ab00; --error: #c0392b; --blue: #4a90e2;
        ...
        html, body, [data-testid="stAppViewContainer"] { background: var(--void); }
        .stApp {
            color: var(--cream); font-family: "Lucida Console", "Courier New", monospace;
            background:
                linear-gradient(90deg, rgba(94,15,32,.18) 1px, transparent 1px) 0 0 / 72px 72px,
                linear-gradient(rgba(200,160,90,.055) 1px, transparent 1px) 0 0 / 72px 72px,
                radial-gradient(circle at 78% 0%, rgba(94,15,32,.25), transparent 29%),
                repeating-linear-gradient(0deg, rgba(255,255,255,.015) 0 1px, transparent 1px 4px),
                #0d0f0f;
        }
        .block-container { max-width: 1420px; padding-top: 1.05rem; padding-bottom: 3rem; }
        #MainMenu, footer { visibility: hidden; }
        h1, h2, h3, h4, p, label, button, input { font-family: "Lucida Console", "Courier New", monospace !important; }
        h1, h2, h3, h4 { color: var(--cream); font-weight: 900; text-transform: uppercase; letter-spacing: -.04em; }
        p, label, [data-testid="stCaptionContainer"] { color: #bdb09b; }
        [data-testid="stSidebar"] {
            background:
                linear-gradient(180deg, rgba(94,15,32,.92) 0 92px, transparent 92px),
                radial-gradient(circle at 110% 14%, rgba(200,160,90,.15), transparent 30%),
                repeating-linear-gradient(0deg, rgba(255,255,255,.012) 0 1px, transparent 1px 4px),
                #151717;
            border-right: 1px solid var(--gold); box-shadow: inset -4px 0 #080909, 8px 0 rgba(94,15,32,.28);
        }
        [data-testid="stSidebar"]:before { content: "IPN  //  SISTEMA TÁCTICO"; display: block; color: #ffe0a7; font-weight: 900; letter-spacing: .08em; font-size: .72rem; padding: 1.2rem 1rem .4rem; }
        [data-testid="stSidebar"] * { color: var(--cream); }
        [data-testid="stSidebar"] h3 { margin-top: .5rem; padding-bottom: .45rem; border-bottom: 1px solid var(--gold); }
        [data-testid="stSidebar"] [data-baseweb="radio"] > div { gap: .3rem; }
        [data-testid="stSidebar"] label[data-baseweb="radio"] {
            position: relative; border: 1px solid #51412d; border-left: 4px solid #51412d; border-radius: 0;
            padding: .48rem .62rem; background: #1a1c1c; clip-path: var(--cut-small);
        }
        [data-testid="stSidebar"] label[data-baseweb="radio"]:hover { color: #ffd69a; border-color: var(--copper); background: #24211d; transform: translateX(3px); }
        [data-testid="stSidebar"] label[data-baseweb="radio"]:has(input:checked) {
            background: linear-gradient(90deg, #72132c, #3c0b19 72%, #1b1919); border-color: var(--gold); border-left: 7px solid var(--gold);
            box-shadow: inset 0 0 0 1px #a4314a, 0 0 14px rgba(94,15,32,.45);
        }
        [data-testid="stSidebar"] label[data-baseweb="radio"]:has(input:checked) > div:first-child { background: var(--gold) !important; border-color: #ffe4ae !important; }
        [data-testid="stSidebar"] label[data-baseweb="radio"]:has(input:checked) > div:first-child > div { background: var(--ipn) !important; }
        [data-testid="stSidebar"] .catalog-strip { display: grid; gap: .48rem; }
        [data-testid="stSidebar"] .catalog-strip div {
            position: relative; padding: .62rem .7rem; border: 1px solid #59462e; border-left: 3px solid var(--copper);
            background: linear-gradient(90deg, #232525, #171919); clip-path: var(--cut-small);
        }
        [data-testid="stSidebar"] .catalog-strip span { color: #e8bd73; font-weight: 900; float: right; }
        .hero {
            position: relative; overflow: hidden; border: 1px solid var(--gold); border-left: 9px solid var(--ipn-hot);
            padding: 1.3rem 1.5rem 1.1rem 2rem; margin-bottom: .9rem; clip-path: var(--cut);
            background:
                radial-gradient(circle at 86% 18%, rgba(184,45,75,.22) 0 2px, transparent 3px) 0 0 / 13px 13px,
                linear-gradient(112deg, #690f29 0 31%, #34101b 31% 48%, #191b1b 48% 100%);
            box-shadow: inset 0 0 0 3px #240713, 0 0 0 1px #5e3b22, 10px 10px 0 rgba(6,7,7,.74);
        }
        .hero:before { content: "8-BIT IPN"; position: absolute; right: 1rem; bottom: .65rem; color: rgba(252,240,220,.09); font-weight: 900; font-size: 2.25rem; letter-spacing: -.08em; }
        .hero.compact { min-height: 162px; padding: 1rem 1.1rem 1rem 1.35rem; margin: 0; }
        .hero.compact .hero-title { font-size: 1.44rem; max-width: 82%; }
        .hero.compact .hero-subtitle { font-size: .78rem; line-height: 1.35; }
        .hero.compact .hero-date { margin-top: .5rem; }
        .hero.compact .badge-row { margin-top: .58rem; gap: .32rem; }
        .hero.compact .badge { padding: .24rem .45rem; font-size: .62rem; }
        .hero:after { content: "SYS / ACTIVO  ◈  8BIT-IPN"; position: absolute; top: 10px; right: 16px; color: #e5b86b; font-size: .54rem; letter-spacing: .07em; }
        .hero-kicker { color: #ffcf82; font-size: .69rem; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; border-bottom: 1px dotted #be7c3e; padding-bottom: .24rem; width: 72%; }
        .hero-title { color: #fff2dc; font-size: 1.78rem; font-weight: 900; margin: .38rem 0 .2rem; text-transform: uppercase; text-shadow: 3px 3px #20030c; }
        .hero-subtitle { color: #cbbca4; font-size: .88rem; margin: 0; }
        .hero-date { margin-top: .75rem; color: #f2d39d; font-size: .86rem; font-weight: 900; }
        .badge-row { display: flex; flex-wrap: wrap; gap: .45rem; margin-top: .85rem; }
        .badge {
            display: inline-flex; align-items: center; gap: .38rem; padding: .32rem .58rem; border-radius: 2px;
            background: #222525; border: 1px solid #7e6645; color: #eadfcb; font-size: .68rem; font-weight: 800;
            clip-path: var(--cut-small); box-shadow: inset 0 -2px #111313;
        }
        .badge-dot { width: 8px; height: 8px; border-radius: 1px; background: var(--green); box-shadow: 0 0 6px rgba(76,175,80,.45); }
        .badge.warn { color: #e8c77e; background: #2a261d; border-color: #a87c28; }
        .badge.warn .badge-dot { background: var(--amber); }
        .badge.ipn { color: #ffe1aa; background: #5e0f20; border-color: var(--gold); }
        .badge.ipn .badge-dot { background: var(--gold); }
        .section-label, .top-control-label { color: var(--copper); font-size: .7rem; font-weight: 900; letter-spacing: .09em; text-transform: uppercase; }
        .section-label { margin: .25rem 0 .75rem; padding: .38rem .65rem; width: fit-content; background: #5e0f20; border: 1px solid var(--gold); clip-path: var(--cut-small); }
        .section-label:before { content: "01 // "; color: #ffe0a2; }
        ...
        .metric-card, .group-card {
            position: relative; overflow: hidden; border: 1px solid #725434; border-radius: 0; clip-path: var(--cut);
            background:
                linear-gradient(135deg, color-mix(in srgb, var(--accent) 17%, #252727) 0 16px, transparent 16px),
                repeating-linear-gradient(0deg, rgba(255,255,255,.012) 0 1px, transparent 1px 4px), #1b1d1d;
            box-shadow: inset 0 0 0 2px #101212, 7px 7px 0 #070808;
        }
        .metric-card { min-height: 118px; padding: 1rem 1rem .8rem 1.1rem; border-left: 5px solid var(--accent); }
        .group-card { padding: .86rem .9rem .78rem; border-bottom: 4px solid color-mix(in srgb, var(--accent) 78%, #30271d); }
        .metric-card:before { content: "◈"; position: absolute; right: 12px; top: 10px; color: var(--accent); font-size: .85rem; }
        .metric-card:after, .group-card:after { content: ""; position: absolute; right: 0; bottom: 0; width: 14px; height: 14px; background: color-mix(in srgb, var(--accent) 70%, #4a2d18); clip-path: polygon(100% 0,100% 100%,0 100%); }
        .metric-label, .group-title { color: #d7cbb8; font-size: .7rem; font-weight: 900; text-transform: uppercase; letter-spacing: .02em; }
        .metric-label { padding-bottom: .32rem; border-bottom: 1px dotted #604e37; }
        .metric-value { color: #f4ead7; font-size: 1.75rem; font-weight: 900; margin-top: .42rem; line-height: 1; text-shadow: 2px 2px 0 #080909; }
        .metric-note, .group-main span, .group-foot { color: #a99c87; font-size: .67rem; }
        .group-title { min-height: 2.1em; border-bottom: 1px dotted #705136; }
        .group-main { display: flex; justify-content: space-between; align-items: baseline; margin-top: .42rem; }
        .group-main strong { color: #f4ead7; font-size: 1.28rem; }
        .group-bar { height: 9px; background: #0b0d0d; border: 1px solid #725434; overflow: hidden; margin: .55rem 0 .45rem; }
        .group-bar span { display: block; height: 100%; background: repeating-linear-gradient(90deg,var(--accent) 0 8px,color-mix(in srgb,var(--accent) 55%,#111) 8px 10px); }
        .group-foot { display: flex; justify-content: space-between; }
        .empty-state { border: 1px solid var(--gold); border-left: 8px solid var(--amber); text-align: left; padding: 1.35rem; color: #cbbb9f; background: #1c1e1e; clip-path: var(--cut); box-shadow: 8px 8px #070808; }
        .empty-state:before { content: "⚠  SISTEMA IPN / ADVERTENCIA"; display: block; margin: -1.35rem -1.35rem 1rem; padding: .42rem .65rem; background: #5e0f20; border-bottom: 1px solid var(--gold); color: #ffe0a4; font-weight: 900; }
        .empty-state strong { display: block; color: #f2e8d5; font-size: 1rem; margin-bottom: .3rem; }
        [data-testid="stMetric"] { background: #202222; border: 1px solid #765232; border-left: 4px solid var(--copper); border-radius: 0; padding: .72rem .82rem; clip-path: var(--cut-small); }
        [data-testid="stMetricValue"], [data-testid="stMetricLabel"] { color: #f2e8d5; }
        [data-testid="stFileUploaderDropzone"] {
            min-height: 75px; border-radius: 0; background: linear-gradient(120deg,#242626,#171919) !important; border: 1px solid var(--gold) !important;
            border-left: 6px solid var(--ipn-hot) !important; padding: .58rem; clip-path: var(--cut); box-shadow: inset 0 0 0 2px #0a0b0b, 7px 7px #080909;
        }
        [data-testid="stFileUploaderDropzone"] small { display: none; }
        [data-testid="stFileUploaderDropzone"] p, [data-testid="stFileUploaderDropzone"] span, [data-testid="stFileUploaderDropzone"] div { color: #eadfcb !important; }
        [data-testid="stFileUploaderFile"], [data-testid="stFileUploaderFile"] > div,
        [data-testid="stFileUploaderFileName"], [data-testid="stFileUploaderFile"] section {
            color: #f2e8d5 !important; background: #252727 !important; border-color: #8b673d !important;
        }
        [data-testid="stFileUploaderFile"] { border: 1px solid #765232 !important; border-left: 5px solid var(--copper) !important; clip-path: var(--cut-small); }
        button, [data-baseweb="select"] > div, [data-baseweb="input"] > div, .stTextInput input { border-radius: 1px !important; }
        button svg, summary svg, [data-baseweb="select"] svg, [data-testid="stSidebarCollapseButton"] svg, [data-testid="collapsedControl"] svg { color: #e8bd73 !important; fill: #e8bd73 !important; stroke: #e8bd73 !important; opacity: 1 !important; }
        button[kind="secondary"], .stDownloadButton button {
            color: #ffe1aa !important; background: linear-gradient(180deg, #6f1429, #4a0d1b) !important;
            border: 1px solid #c8a05a !important; border-left: 5px solid #e4b761 !important; clip-path: var(--cut-small); box-shadow: inset 0 0 0 1px #8f203a; font-weight: 900 !important;
        }
        button[kind="secondary"]:hover, .stDownloadButton button:hover { color: #fff1ce !important; background: linear-gradient(180deg, #92203e, #5e0f20) !important; filter: brightness(1.12); }
        button[kind="secondary"]:active, .stDownloadButton button:active { transform: translate(2px,2px); }
        [data-baseweb="select"] > div, [data-baseweb="input"] > div, .stTextInput input { color: #f2e8d5 !important; background: #202222 !important; border: 1px solid #765232 !important; box-shadow: inset 2px 2px 0 #0b0c0c; }
        [data-baseweb="select"] > div:focus-within, [data-baseweb="input"] > div:focus-within, .stTextInput input:focus { border-color: #d79a4e !important; box-shadow: inset 2px 2px 0 #0b0c0c, 0 0 0 1px #c8a05a, 0 0 10px rgba(215,154,78,.34) !important; }
        [data-baseweb="popover"], [data-baseweb="menu"], [role="listbox"] { color: #f2e8d5 !important; background: #202222 !important; border: 1px solid #c8a05a !important; border-radius: 0 !important; box-shadow: 7px 7px 0 #080909 !important; }
        [role="option"] { color: #f2e8d5 !important; background: #202222 !important; border-bottom: 1px solid #403626; }
        [role="option"]:hover, [role="option"][aria-selected="true"] { color: #ffe3a8 !important; background: #5e0f20 !important; }
        [data-baseweb="tab-list"] { gap: 6px; background: transparent; border-bottom: 1px solid #765232; padding: 0 0 .35rem; border-radius: 0; }
        [data-baseweb="tab"] { color: #cbbda6; height: 40px; border-radius: 0; padding: 0 1rem; border: 1px solid #59462e; background: #191b1b; clip-path: var(--cut-small); }
        [aria-selected="true"][data-baseweb="tab"] { color: #ffe4ac; background: linear-gradient(90deg,#72132c,#3a0b18); border-color: #c8a05a; border-left: 6px solid var(--gold); }
        [data-baseweb="tab-highlight"] { display: none; }
        [data-testid="stDataFrame"] { border: 1px solid #9a713f; border-left: 6px solid var(--ipn-hot); border-radius: 0; overflow: hidden; background: #202222 !important; clip-path: var(--cut); box-shadow: 8px 8px #070808; }
        [data-testid="stArrowVegaLiteChart"] { border: 1px solid #9a713f; border-top: 5px solid var(--gold); border-radius: 0; padding: .55rem; background: #171919; clip-path: var(--cut); box-shadow: 8px 8px #070808; }
        details { border: 1px solid #765232 !important; border-left: 5px solid var(--copper) !important; border-radius: 0 !important; background: #202222 !important; clip-path: var(--cut-small); }
        details summary, div[data-testid="stExpander"] > details p, div[data-testid="stExpander"] > details label, div[data-testid="stExpander"] > details span { color: #f2e8d5 !important; }
        [data-testid="stVerticalBlockBorderWrapper"] { position: relative; border: 1px solid #765232 !important; border-radius: 0 !important; background: linear-gradient(145deg,#1e2020,#141616); clip-path: var(--cut); box-shadow: 8px 8px #070808; }
        [data-testid="stVerticalBlockBorderWrapper"]:before { content: "TACTICAL DATA MODULE"; display: block; color: #d79a4e; font-size: .55rem; letter-spacing: .08em; border-bottom: 1px dotted #765232; padding: .35rem .65rem; }
        [data-testid="stVerticalBlockBorderWrapper"] h4 { color: #f2e8d5; }
        .panel-kicker { display: inline-block; border-radius: 0; padding: .32rem .62rem; margin-bottom: .3rem; font-size: .62rem; font-weight: 900; letter-spacing: .07em; text-transform: uppercase; border: 1px solid #c8a05a; clip-path: var(--cut-small); }
        .panel-kicker.sage { color: #c9f0cb; background: #214327; border-color: #4caf50; }
        .panel-kicker.clay { color: #ffe1aa; background: #5e0f20; border-color: #c8a05a; }
        .panel-kicker.mauve { color: #f4c8d2; background: #4a1522; border-color: #9b4156; }
        .query-count { color: #b9aa93; font-size: .76rem; margin: -.15rem 0 .55rem; }
        .stAlert { border-radius: 0; background: #202222; border: 1px solid #765232; border-left: 10px solid #e0ab00; color: #f2e8d5; clip-path: var(--cut-small); }
...
def style_query_rows(row: pd.Series) -> list[str]:
    ...
    if globals().get("visual_theme") == "8-bit IPN":
        backgrounds = {
            "Con checada": "background-color: #17291b; color: #d8f0d9; border-left: 4px solid #4caf50",
            "Sin checada": "background-color: #332a17; color: #f4dda5; border-left: 4px solid #e0ab00",
            "No encontrado / revisar": "background-color: #382719; color: #f0cc82; border-left: 4px solid #d79a4e",
            "Ambiguo / revisar": "background-color: #3b1420; color: #f1c5cf; border-left: 4px solid #c0392b",
        }
        style = backgrounds.get(state, "background-color: #191b1b; color: #f2e8d5; border-left: 4px solid #76502c")
            ...
            **{
            ...
            [
                ...
                },
                {
                    ...
                    "props": [("background-color", "#222424")],
                },
            ]
        )
    elif globals().get("visual_theme") == "Pintado claro":
        styler = styler.set_properties(
            **{
...
with st.sidebar:
    ...
    visual_theme = st.radio(
        "Tema visual",
        ["8-bit IPN", "Pintado claro", "Studio táctil", "Obra vigente"],
        index=0,
        key="visual_theme",
    )
            ...
            if not chart_data.empty:
                ...
                if visual_theme == "8-bit IPN":
                    ...
                    chart = (
                        ...
                        .mark_bar(stroke="#c8a05a", strokeWidth=1)
                        .encode(
                            x=alt.X("grupo:N", title=None, sort=None, axis=alt.Axis(labelAngle=-55, labelLimit=120)),
                            y=alt.Y("personas:Q", title=None, stack="zero", axis=alt.Axis(gridColor="#493b2b")),
                            color=alt.Color(
                                "estado:N",
                                title=None,
                                scale=alt.Scale(
                        ...
                        .properties(height=300, background="#171919")
                        .configure_view(stroke="#c8a05a", strokeWidth=1)
                        .configure_axis(
                            labelColor="#d8cbb7",
                            domainColor="#c8a05a",
                            tickColor="#c8a05a",
                        ...
                        .configure_legend(labelColor="#d8cbb7", labelFont="Lucida Console", orient="bottom")
                    )
                    st.altair_chart(chart, use_container_width=True)
                elif visual_theme == "Pintado claro":
                    chart_long = chart_data.melt(
                        id_vars="grupo",
                        value_vars=["con_checada", "sin_checada"],
                        var_name="estado",
